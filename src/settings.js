...
export const SPEC = [
  ...
  },
  {
    ...
    hint: 'Der echte Abstand wird pro Send zufällig zwischen Minimum und Maximum gewählt. Unter 120 s wird es ein Burst-Muster.',
    type: 'int',
    min: 0,
    max: 14400,
  ...
  },
  {
    ...
    hint: 'Die dritte unbeantwortete Nachricht holt die Spam-Reports. 2 ist ein guter Wert. Varianten mit höherem Step werden nie gezogen, solange dieser Wert kleiner ist.',
    type: 'int',
    min: 1,
    max: 5,
  ...
  // ── Bremsen ─────────────────────────────────────────────────────────────
  {
    ...
    hint: 'Fällt die Rate darunter, pausiert die Kampagne selbst. Das ist kein Ziel, sondern ein Alarm — 0.10 ist für Kaltkontakt sinnvoll.',
    type: 'float',
    min: 0,
    max: 1,
  ...
  },
  {
    ...
    hint: 'Verhindert, dass ein langsamer Start sofort pausiert. Darf nicht grösser als das Fenster sein — die Stichprobe kann nie grösser werden als das, worüber gerechnet wird.',
    type: 'int',
    min: 1,
    max: 500,
  ...
  {
    key: 'replyRateMaturityHours',
    group: 'Bremsen',
    label: 'Antworten reifen lassen (Stunden)',
    hint: 'Sends, die jünger sind, zählen nicht in die Reply-Rate. Bei Kaltkontakt kommen Antworten über Tage — ohne Reifezeit pausiert die Bremse wegen Nachrichten, die noch niemand gelesen hat. 0 schaltet es ab.',
    type: 'int',
    min: 0,
    max: 336,
    def: () => config.guard.replyRateMaturityHours,
  },
